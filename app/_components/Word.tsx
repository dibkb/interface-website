...
const Word = ({ children, progress, range }: WordProps) => {
    ...
    return (
        <div className="flex items-center">
            <span className="text-white text-xl sm:text-3xl xl:text-5xl font-bold leading-[30px] xl:leading-[65.08px] relative mr-[12px] xl:mt-[12px]">
                <span className={"absolute opacity-20"}>{children}</span>
                <motion.span style={{ opacity: opacity }}>{children}</motion.span>
            </span>
            {children === "businesses" &&
                <span className="text-white text-xl sm:text-4xl xl:text-6xl font-bold leading-[65.08px] relative mr-[12px]">
            ...
            {children === "efficiency" &&
                <span className="text-white text-xl sm:text-4xl xl:text-6xl font-bold leading-[65.08px] relative mr-[12px]">
                    <span className={"absolute opacity-20 text-xl sm:text-3xl xl:text-5xl"}><FaRegClock /></span>
                    <motion.span style={{ opacity: opacity }} className="text-violet-700 text-xl sm:text-3xl xl:text-5xl"><FaRegClock /></motion.span>
                </span>
            }
        </div>
    )
